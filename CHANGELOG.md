...
### Added
- **UI visual refresh**: New design token system across the web dashboard — refined dark palette with subtle radial backgrounds, gradient-filled stat cards with hover lift, brand-accent underlines on panel headers, zebra-striped tables with sticky blurred headers, FE icon glow, pill-shaped version badge, and softer modals with backdrop blur. Chart.js styling modernized with line gradients, slim grid, doughnut cutout, and rounded tooltips. Numeric stats now use tabular figures.
- **Chart zoom & pan**: The two dashboard line charts now support mouse-wheel zoom (X-axis), Shift+drag to pan, Ctrl+drag to box-zoom, pinch zoom on touch, and double-click to reset. Backed by `chartjs-plugin-zoom`.
- **Persistent icon cache on disk**: The `/api/icons/{id}` proxy now caches downloaded icons under `<data dir>/icon_cache/` in addition to the in-memory cache. Icons survive restarts and only have to be fetched from the CDN once.
...
- **Independent per-panel rendering in `refreshAll`**: The dashboard previously waited for all 8 API requests to complete (`Promise.all`) before rendering anything. A single slow request (e.g., 24h stats history, cloud status) would stall the whole UI. Each panel now renders the moment its own fetch resolves, so the Current Map / status indicator update is no longer blocked by heavier panels.
...
### Fixed
- **"Check for Updates" button felt unresponsive**: The button is at the bottom-right of the footer, but the only feedback after clicking was a small toast in the *top-right* corner that flashed for 3 s. With the click and the response on opposite ends of the page, it was easy to think the button wasn't doing anything. Now the button itself updates immediately on click (`Checking…`, disabled), then briefly displays `✓ Up to date` / `Check failed` for ~2.5–3 s before reverting. The corresponding "you're on the latest version" toast also stays for 5 s instead of 3 s.
- **Current Map icons never loading after a fresh pickup**: `renderActiveRun()` included `duration_seconds` in its diff hash, but the duration ticks up every refresh. The hash therefore mismatched on every tick, triggering a full `innerHTML = …` rebuild of the loot list. Replacing the DOM cancels any in-flight `<img>` requests on the old nodes, so each icon's CDN fetch was aborted before it could finish — only items already cached by the browser ever appeared. Duration is now updated in place via `textContent` and excluded from the hash, so freshly picked items show their icon the moment the proxy returns.
- **Icon proxy permanently blacklisted failed URLs**: A single transient network timeout would write the icon's URL into an in-process `_failed_urls` set forever, hiding that icon until the next app restart. Failures are now stored with a timestamp; HTTP 4xx (except 408/429) stay permanent, everything else retries after 60 seconds. The browser-side `failedIcons` set was changed from `Set` to `Map<id, ts>` with the same TTL behaviour.
- **Cloud prices missing on first paint**: When the dashboard polling loop was restructured to render each panel independently, the inventory and stats panels could render before `loadCloudPrices()` finished, briefly showing items without their community prices / sparklines. The cloud-status branch now re-runs `renderInventory()` and `renderStats()` once the price cache has been populated.
- **Icon disk cache could be left corrupted by a crash mid-write**: Icons are now written via a temp file + atomic `replace()`, so a process kill (or power loss) during the write never leaves a partial file behind that would later be served as a broken image.
